function [S1_vt, S2_vt] = texture_transfer_pMap(T12, S1, S2, B1, B2, save_path)
B2_inv = pinv(B2);
B1_inv = pinv(B1);
% T12: maps S1 -> S2, n1-dim vector
n1 = length(S1.surface.X);
n2 =  length(S2.surface.X);
P21 = sparse(1:n1, double(T12), ones(1,n1), n1, n2); % n1-by-n2 matrix, maps S2 -> S1
...
texture_im = 'texture2.png';
X2 = S2.surface.VERT;
[~,ic] = sort(std(X2),'descend');
%ic = [1,2,3];
S2_vt = MESH.MESH_IO.generate_tex_coords(X2(:,ic), cols(2), cols(1), 1);

S2_vt(:,1) = (S2_vt(:,1) - min(S2_vt(:,1)))/(max(S2_vt(:,1)) - min(S2_vt(:,1)));
S2_vt(:,2) = (S2_vt(:,2) - min(S2_vt(:,2)))/(max(S2_vt(:,2)) - min(S2_vt(:,2)));
...
S1_vt = P21*S2_vt;
% Target mesh: 
MESH.MESH_IO.wobj_with_texture(S1, S1_vt, texture_im, [save_path,S1_name]);
